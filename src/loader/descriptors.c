...
const uint8_t gu8VendorStringDesc[] =
{
    18,
    DESC_STRING,
    'R', 0, 'o', 0, 's', 0, 'e', 0, 'f', 0, 'i', 0, 's', 0, 'h', 0
};

/*!<USB Product String Descriptor */
uint8_t gu8ProductStringDesc[64] =
{
    30,
    DESC_STRING,
    'E', 0, 'Z', 0, '-', 0, 'C', 0, 'r', 0, 'e', 0, 'a', 0, 't', 0, 'o', 0, 'r', 0, ' ', 0, 'P', 0, 'a', 0, 'd', 0
};

void ProductStringDescInit(void)
{
    uint8_t size = gu8ProductStringDesc[0];
    uint8_t *dest = gu8ProductStringDesc + size;

    *dest++ = ' ';
    *dest++ = 0;
    size += 2;

    uint8_t *source = (uint8_t *)bootloader_name;
    while (*source)
    {
        if (size+2 > sizeof(gu8ProductStringDesc))
            break;
...
void StringVersionInit(void)
{
    uint8_t size = 2;
    uint8_t *dest = gu8StringVersion + size;
    
    uint8_t *source = (uint8_t *)bootloader_version;
    while (*source)
    {
        if (size+2 > sizeof(gu8StringVersion))
            break;
...
void DescriptorsInit(void)
{
    ProductStringDescInit();
    StringVersionInit();
}
