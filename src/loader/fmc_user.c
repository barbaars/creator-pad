...
{
    return FMC_Proc(FMC_ISPCMD_READ, u32Addr, u32Addr + 4, (unsigned int *)data);
}

void ReadData(uint32_t addr_start, uint32_t addr_end, uint32_t *data)    // Read data from flash
{
    FMC_Proc(FMC_ISPCMD_READ, addr_start, addr_end, (unsigned int *)data);
    return;
}

void WriteData(uint32_t addr_start, uint32_t addr_end, uint32_t *data)  // Write data into flash
{
    FMC_Proc(FMC_ISPCMD_PROGRAM, addr_start, addr_end, (unsigned int *)data);
    return;
}
